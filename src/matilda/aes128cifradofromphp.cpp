...
QByteArray AES128CifradoFromPhp::aes128_ecb_encrtypt(const QByteArray &arrEncr, const QByteArray &arrKey)
{
    ...
    QByteArray retArr("");
    for(int i = 0, iMax = arrEncr.length(); i < iMax; i += 16){
        uint8_t output[16];
        for(int j = 0; j < 16; j++)
            output[j] = (uint8_t)arrEncr.at(j + i);
        ...
        for(int j = 0; j < 16; j++)
            retArr.append((uint8_t)output[j]);
...
QByteArray AES128CifradoFromPhp::aes128_ecb_decrtypt(const QByteArray &arrDecr, const QByteArray &arrKey)
{
    ...
    QByteArray retArr("");
    for(int i = 0, iMax = arrDecr.length(); i < iMax; i += 16){
        uint8_t output[16];
        for(int j = 0; j < 16; j++)
            output[j] = (uint8_t)arrDecr.at(j + i);
...
//    QByteArray retArr("");
    for(int i = 0, iMax = listEncr.length(); i < iMax; i += 16){
        uint8_t output[16];
        for(int j = 0; j < 16; j++)
            output[j] = (uint8_t)listEncr.at(j + i);
        ...
        for(int j = 0; j < 16; j++)
            retList.append((quint8)output[j]);
...
//===================================================================================================================================
QList<quint8> AES128CifradoFromPhp::aes128_ecb_decrtypt(const QList<quint8> &listDecr, const QByteArray &arrKey)
{
    QList<quint8> retList;
...
//    QByteArray retArr("");
    for(int i = 0, iMax = listDecr.length(); i < iMax; i += 16){
        uint8_t output[16];
        for(int j = 0; j < 16; j++)
            output[j] = (uint8_t)listDecr.at(j + i);
...
//===================================================================================================================================
QByteArray AES128CifradoFromPhp::keyEncrKey(const QByteArray &arrKey)
{
    QByteArray arr = validPasswd(arrKey);
    return aes128_ecb_encrtypt(arr,arr);
}
//===================================================================================================================================
QByteArray AES128CifradoFromPhp::keyEncrKey(const QByteArray &arrKey, const int &raziv)
{
    QByteArray arr = validPasswd(arrKey);
    QByteArray rez = arr;
    for(int i = 0; i < raziv && i < 11; i++)
        rez = aes128_ecb_encrtypt(rez, arr);
    return rez;
}
//===================================================================================================================================
bool AES128CifradoFromPhp::keyAndKeyGood(const QByteArray &arrKey1Hash, const QByteArray &arrKey2Enrc, const int &raziv)
{
    QByteArray rez = arrKey1Hash;
    for(int i = 0; i < raziv && i < 11; i++){
        rez = aes128_ecb_encrtypt(rez, arrKey1Hash);
        if(rez == arrKey2Enrc)
            return true;
    }
    return false;
...
//===================================================================================================================================
QByteArray AES128CifradoFromPhp::validLogin(const QString &login)
{
    return validLogin(login.toUtf8());
}
//===================================================================================================================================
QByteArray AES128CifradoFromPhp::validLogin(const QByteArray &login)
{
...
//===================================================================================================================================
QByteArray AES128CifradoFromPhp::validPasswd(const QString &paswrd)
{
    return validPasswd(paswrd.toUtf8());
}
//===================================================================================================================================
QByteArray AES128CifradoFromPhp::validPasswd(const QByteArray &paswrd)
{
    QCryptographicHash h(QCryptographicHash::Md5);
    h.addData(paswrd);
    QByteArray arr = h.result().toHex().rightJustified(32, '0');


//    QByteArray arr("");

//    QByteArray arrH = QByteArray::fromHex(paswrd);

//    if( paswrd.toLower() == arrH.toHex().toLower()){
//        qDebug() << "validPasswd arr not hex";
//        arr = paswrd;
//    }else
//        arr = paswrd.toHex();
...
//===================================================================================================================================
void AES128CifradoFromPhp::AES128_ECB_decrypt(uint8_t *input, const uint8_t *key, uint8_t *output)
{
    // Copy input to output, and work in-memory on output
      BlockCopy(output, input);
      state = (state_t*)output;
